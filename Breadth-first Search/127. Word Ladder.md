...
* **the most votes**
  * **BFS** `Runtime: 90 ms, faster than 43.70%, Memory Usage: 47.8 MB, less than 19.47% of Java online submissions`
    ```
  ...
  * **Bidirectional BFS** `Runtime: 42 ms, faster than 82.77%, Memory Usage: 52.4 MB, less than 12.07% of Java online submissions`
    ```
    //O(M^2 * N)time O(M^2 * N)space
    //M is word.length, N is list.size()
    private int L;
