...
* **mine**
  * **Java**
		...
		* **record offset** `Runtime: 846 ms, faster than 52.80%, Memory Usage: 41.1 MB, less than 100.00% of Java online submissions`
			```
			// O(N^2)time
			// O(N)space
			...
			public String minInteger(String num, int k) {
					...
					while (k > 0 && count < len) {
							for (int i = 0; i < 10; i++) {
									...
									record.add(index, list[i].removeFirst());
									count++;
									k -= record.get(index) - index;
									res.append(i);
									arr[record.get(index)] = 0;
									continue outer;
							}
					}
					for (int i = 0; i < len; i++) {
